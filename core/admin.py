...
def payout_to_courier(modeladmin, request, queryset):
    ...
    #Step 2 - Send payout batch + email to the recievers
    ...
    # Step 3 - Execute Payout process & Update the transaction status to "OUT" if success 

    try:
        if payout.create():
            for t in transaction_querysets:
                t.update(status=Transaction.OUT_STATUS)
            messages.success(request, "Payout[%s] created succesfully." % (payout.batch_header.payout_batch_id))
...
class TransactionAdmin(admin.ModelAdmin):
    list_display = ['stripe_payment_intent_id','courier_paypal_email','courier','job','customer','courier','amount','status','created_at']
    list_filter = ['status',]
